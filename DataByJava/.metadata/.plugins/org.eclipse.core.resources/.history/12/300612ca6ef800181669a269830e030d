...
<body class="page-header-fixed" >
    ...
    <div class="page-container row-fluid">
        ...
        <th width="200">用户心声</th>
        <th width="200">邮箱地址</th>
        <th width="200">操作</th>
        </tr>
        </thead>
        <tbody>
		<c:forEach items="${pages.users}" var="users">
        <tr>
        <td><input name="" type="checkbox" value="" /></td>
        <td width="200">${users.id}</td>
        <td width="200">${users.user_name}</td>
        <td width="200">${users.user_think}</td>
        <td width="200">${users.user_email}</td>
        <td width="200">     
        <a href="deleteUser.do?method=delete&key=${users.id}" class="tablelink"  onclick="return confirm('确认删除吗?')"> 删除</a></td>
        </tr> 
		</c:forEach>
        </tbody> 
    </table>
    ...
    <div class="pagin" style="text-align: center;margin-top:100px">
		...
		<a href="userShow.do?currentPage=1">首页</a>
		<c:if test="${currentPage != 1 }">
		<a href="userShow.do?currentPage=${currentPage-1 }">上一页</a>
		</c:if>
		<c:if test="${currentPage != pages.totalPageCount }">
		<a href="userShow.do?currentPage=${currentPage+1 }">下一页</a>
		</c:if>
		<a href="userShow.do?currentPage=${pages.totalPageCount}">尾页</a>
		<span>当前第${currentPage}页</span>
   		<span>共${pages.totalPageCount}页</span>
	
		<div class="message" style="text-align: center">共<i class="blue">${pages.userCount}</i>条记录，当前显示第&nbsp;<i class="blue">${currentPage}&nbsp;</i>页</div>
    	<form action="userShow.do">
